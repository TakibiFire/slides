---
fonts:
  # 標準テキスト用
  sans: Noto Sans JP
  # UnoCSS で `font-serif` クラスを指定したとき用
  serif: Noto Serif JP
  # コードブロック用
  mono: Fira Code
title: FIRE後の友達の作り方 | FIRE後の教科書 | たきび FIRE
titleTemplate: "%s"
favicon: https://takibi-fire.com/slides/public/imgs/favicon.png
highlighter: shiki
css: unocss
colorSchema: dark
transition: fade
selectable: true
mdc: true
layout: center
lang: ja
glowSeed: 4
footerTitle: FIRE後の教科書 > 役立つ知識・考え方 > FIRE後の友達の作り方
---

<style scoped>
h1 {
  font-size: 68px;
  line-height: 150%;
}
h1, p {
  text-align: center;
}
</style>

# FIRE後の友達の作り方

<div text-center mb-8 font-bold bg-gradient-to-r from-indigo-200 via-blue-500 to-purple-400 text-transparent bg-clip-text>
  <span text-13>FIRE後の教科書</span><br>
  <span text-8>役立つ知識・考え方</span>
</div>

<div text-center grid grid-cols-2 gap-4 m-auto w-fit>
  <div col-span-2>たきび FIRE</div>
  <div>
    <a href="http://note.com/takibi333">
      <img src="https://takibi-fire.com/slides/public/imgs/note-logo.svg" h8 inline />
    </a>
  </div>
  <div>
    <a href="http://x.com/takibi333">
      <img src="https://takibi-fire.com/slides/public/imgs/x-logo.svg" h8 inline />
    </a>
  </div>
</div>

<img src="https://takibi-fire.com/slides/public/imgs/takibi.png" absolute w24 right-8 bottom-8 />

---

# 1枚で要約

<div grid grid-cols-12 gap-6><div col-span-7 text-4.9>

* FIREすると元の仕事仲間と同じ関係ではいられない
  * 元同僚からの受け取られ方が変わる
* 新しいコミュニティでの友達の作り方のヒント
  * ただ「ぶらぶらする」のではなく、<br>一緒にする活動に焦点を当てる
  * 話すよりも聞く・アドバイスを求める
  * 贈り物や好意を与る
  * 挑戦し続ける
* [この話はnoteの記事にもしています](https://note.com/takibi333/n/n1a4abdd5924a)

</div><div col-span-5 text-base>

<fieldset border rounded-3 p-2>
  <legend>自己紹介</legend>

<div flex flex-row>
  <div flex-0 mr-3>
    <img src="https://takibi-fire.com/slides/public/imgs/takibi.png" w24 />
  </div>
  <div my-auto>
    <ul>
      <li>たきび (@takibi333)</li>
      <li>40代</li>
      <li>子供3人</li>
    </ul>
  </div>
</div>

- 東大 → GAFAM エンジニア → 2024 年 FIRE
- 運動 1/3, 勉強 1/3, 育児 1/3
- FI = FIRE したから資産のことは気にしない
- RE = Retire Experiment! (リタイア実験)
- 65 歳に「強くてニューゲーム」したい
- 詳しくは[自己紹介](https://takibi-fire.com/slides/fire-self-introduction/)

</fieldset>

</div></div>

<div text-base absolute bottom-8 left-8>スライド一覧を流し見したい人は左下から<carbon-apps />をクリック
</div>

---

# この話のきっかけ

<div grid grid-rols-2 gap-8><div>

[Retiring Early? Don’t Expect Your Friends to Follow You](https://www.millennial-revolution.com/freedom/retiring-early-dont-expect-your-friends-to-follow-you/)<br>
(訳: 早期リタイア？友達がついてくるとは思うな)という記事が<br>面白かったし共感したので紹介します

</div><div>

<img src="/imgs/article.png" alt="記事のスクリーンショット" class="w-11/20" mx-auto/>

</div></div>

---

<style scoped>
.slidev-layout {
  font-size: 140%;
}
</style>

# FIRE後<span text-orange-500>あなたは仕事の友人を失う</span>

* 言いすぎかもしれませんが少なくともそういう覚悟でいましょう
* 仕事の友人は**職場**という環境で繋がっていた
  * 「あなたが誰であるか」ではなく「あなたがどこにいるか」で繋がっていた
* そして**共通の話題**で繋がっていた
  * 会社の愚痴、ストレス、締切、...
* FIRE後その両方を失います

<fieldset border rounded-3 px-6 pb-2 mt-2 w-fit mx-auto text-5>
  <legend ml-10 color-green>
  <img src="https://takibi-fire.com/slides/public/imgs/takibi.png" inline w8 />
  たきび体験談
</legend>

* FIRE後に仕事仲間から愚痴を聞いても「大変だねぇ」としか言えない
* アドバイスするのも違う
* 「だったら辞めちゃえば？」とも言えない

</fieldset>


---

# FIRE後<span text-orange-500>元同僚からの見られ方が変化する</span>

<div border p-2 w-fit mb-3 mx-auto rounded-2>

あなたが何も変化していなくても、**元同僚からは「共感できない」存在になる**

</div>

* あなたが自由な暮らしを見せつければ、**元同僚は不自由さを直視しなくてはいけなくなる**
  * 直視するのが辛いと「あなたの人生には自分はもう興味がない」という<br>反応をする人もいるでしょう


---

# 直面している問題が違いすぎる

<div grid grid-cols-2 gap-10><div>

<fieldset border rounded-3 px-6 pb-2>
  <legend ml-10 color-green>あなたの悩み</legend>

* 自由の謳歌の仕方
* 自分探し・自己実現
* 趣味

</fieldset>

</div><div>

<fieldset border rounded-3 px-6 pb-2>
  <legend ml-10 color-red>彼らの悩み</legend>

* 締め切り、つまらない仕事
* 上司・部下
* 育児

</fieldset>

</div></div>

<div w-fit m-auto text-center>

## **話が噛み合わない**

自分から相手に共感できるつもりでも<br>相手から自分への共感が下がる

自分の話の何が自慢と取られるか分からない

</div>

---

# もしあなたがFIRE達成者なら<br>FIRE前の自分と話が合うか考えてみよう

<div grid grid-cols-2 gap-10><div>

<fieldset border rounded-3 px-6 pb-2>
  <legend ml-10 color-red>
  <img src="https://takibi-fire.com/slides/public/imgs/takibi.png" inline w8 hue-rotate-210 />
  昔のたきび
  </legend>

* 「自己実現」とか語っちゃう人ダサい
* プロジェクトに突っ走りたい

</fieldset>

</div><div>

<fieldset border rounded-3 px-6 pb-2>
  <legend ml-10 color-green>
  <img src="https://takibi-fire.com/slides/public/imgs/takibi.png" inline w8 />
  今のたきび
  </legend>

* 自分が何をしたら楽しいか探したい
* ゆっくり寄り道を楽しみたい

</fieldset>

</div></div>

<div w-fit m-auto text-center>

## **過去の自分と今の自分でも話が噛み合わなさそう**

</div>

---
layout: fact
---

# 新しい友人を<br>見つけよう

---

# 新しい友人の見つけ方

**簡単ではない**

大人になってから友人を作るのは、子供の頃よりもずっと難しい。

仕事の愚痴を言い合えないとなると、さらに難しい。

しかし、不可能ではない。

<div border px-6 mt-6 rounded-4 text-center w-fit mx-auto color-amber>

FIRE後の新しい価値観に合い<br>自由な時間をたっぷり持つ新しい友人を見つけよう

</div>

---
layout: section
---

# 新しい友人を見つけるヒント

コミュニティを見つけよう

ただぶらぶらするのではなく、活動に焦点を当てよう

話すよりも聞こう

アドバイスを求めよう

贈り物や好意を与えよう

挑戦し続けよう

---

# コミュニティを見つけよう

同じ価値観を持ちながらも、新しい視点を得られるようなコミュニティ

* FIRE コミュニティ
* 地域のコミュニティ、休職者のコミュニティ、趣味のコミュニティ

**複数居場所があるのが理想的**

<div class="w-2/3" border p-2 rounded-4>

「会社での人間関係を理由に FIRE したのに、<br>また違うコミュニティに属して疲弊したくない」

→ 「緩いコミュニティを探しましょう」

</div>


---

# ただぶらぶらするのではなく、活動に焦点を当てる

見知らぬ人に会う場合は、**何か「一緒にすること」を選びましょう。**

オフ会で語り合うのもいいですが、何か一緒に活動や作業をする方が気の合う人を見つけやすいです。

できれば消費的な活動（ゲームや鑑賞）よりも生産的な活動が良いです。

* 運動
* 料理
* 学び

---

# 話すよりも聞く

FIRE したての時は、どうしても話したくなりますよね！
* 達成の話を話すのは嬉しいです
* 他の人に羨ましがられるかもしれません
* 「会社の人に理解されにくい」FIRE話を聞いてくれるFIRE仲間は素敵な存在

**でも、それを分かった上でその逆を行きましょう**。

FIRE の先輩はたくさんいるので、学びましょう。

<div w-fit m-auto text-center text-amber-500>

## 質問をし、相手に好奇心を持ちましょう

</div>

---

# アドバイスを求めよう

聞き上手からさらに一歩進んで、引き出し上手になりましょう。

* 人は他人にアドバイスを与え、賢いと感じるのが好きです。
* アドバイスを求めると、場は和み、**相手は良い気分になり、** あなたは彼らをよりよく知ることができます。
* 一つ聞くとその派生の情報まで色々と聞けるのが FIRE コミュニティの良いところです。

---

# 贈り物や好意を与える

友人を作る簡単な方法は、単に贈り物をしたり、相手のために何かしてあげたりすることです。

FIREできるあなたは何かしらスキルがあるはずです。

今までは会社のために貢献して対価をもらっていましたが、あなたはもう報酬や見返りの無い世界にやってきました。

自発的に**仲間を手伝ってみたり、助けてみたりしましょう。**

自分の得意なことを**報酬や見返りを気にせずやってみましょう。**

---

# 挑戦し続ける

生涯の友達を見つけるのは試行錯誤が必要。

**場数が大事です。**

友人づくりの最初はぎこちなくなるかもしれませんが、<br>こういうのは結局は場数です。

---

# まとめ

* FIRE すると会社の人と同じ関係ではいられなくなります
  * 自分の受け取られ方が変わるし、共感もされにくなります
  * 昔の自分でさえ、今の自分に共感できないかもしれません
* 新しい友人を見つけましょう
  * 価値観が合うコミュニティを見つけましょう
  * 与える側に回ってみましょう
  * 場数が大事です